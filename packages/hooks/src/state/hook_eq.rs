...
#[derive(Debug)]
pub struct StateEq<'a, T: PartialEq, const N: usize = STAGING_STATES_DEFAULT_STACK_COUNT>(
    State<'a, T, N>,
);

impl<'a, T: PartialEq, const N: usize> Default for StateEq<'a, T, N> {
    #[inline]
    fn default() -> Self {
...
impl<'a, T: PartialEq, const N: usize> Unpin for StateEq<'a, T, N> {}

pub struct StateEqWith<'a, T: PartialEq, const N: usize = STAGING_STATES_DEFAULT_STACK_COUNT>(
    State<'a, T, N>,
);
...
crate::utils::impl_hook! {
    impl ['a, T: PartialEq, const N: usize] for StateEq<'a, T, N> {
        #[inline]
...
crate::utils::impl_hook! {
    impl ['a, T: PartialEq, const N: usize] for StateEqWith<'a, T, N> {
        #[inline]
        poll_next_update(mut self, cx) {
            self.0.poll_next_update_if_not_equal(cx, PartialEq::eq)
        }
...
#[inline]
pub fn use_state_eq<'a, T: PartialEq>() -> StateEq<'a, T> {
    Default::default()
}

#[inline]
pub fn use_state_eq_n<'a, T: PartialEq, const N: usize>() -> StateEq<'a, T, N> {
    Default::default()
}
